...
Utilizando la función ADDDATE y la función DATEDIFF de MySQL.*/
select codigo_pedido, codigo_cliente, fecha_esperada, fecha_entrega
	from pedido
	where datediff(fecha_esperada, adddate(fecha_entrega, interval 2 day)) >= 2; 
...
select * from pedido where monthname(fecha_entrega) = 'January'
...
select distinct forma_pago'Formas de pago' from pago;
...
select distinct c.nombre_cliente'Nombre cliente',
	e.nombre'Nombre representante',
	o.ciudad'Ciudad del representante'
...
representantes junto con la ciudad de la oficina a la que pertenece el representante.*/
select c.nombre_cliente'Nombre cliente',
	e.nombre'Nombre representante',
	o.ciudad'Ciudad del representante'
...
select distinct o.linea_direccion1'Dirección oficina' from oficina o
	inner join empleado e
...
jefes.*/
select e.nombre'Nombre empleado', em.nombre'Nombre jefe' from empleado e 
	inner join empleado em 
	on e.codigo_jefe = em.codigo_empleado;
 
/*9. Devuelve el nombre de los clientes a los que no se les ha entregado a tiempo un
pedido.*/
...
/*10. Devuelve un listado de las diferentes gamas de producto que ha comprado cada
cliente.*/
select pr.gama 'Gama del producto' from producto pr 
	...
	inner join pedido pe on dp.codigo_pedido = pe.codigo_pedido
		where pr.codigo_producto in (select dp.codigo_producto from detalle_pedido dp
			inner join pedido p 
...
ningún pedido.*/
select c.* from pedido p right join cliente c 
...
oficina asociada.*/
select e.* from empleado e left join oficina o
	on e.codigo_oficina = o.codigo_oficina
		where o.codigo_oficina is null;
 
/*5. Devuelve un listado que muestre solamente los empleados que no tienen un cliente
asociado.*/
select e.* from cliente c right join empleado e 
	on c.codigo_empleado_rep_ventas = e.codigo_empleado
...
select e.* from oficina o right join empleado e
	on o.codigo_oficina = e.codigo_oficina
	left join cliente c
	on e.codigo_empleado = c.codigo_empleado_rep_ventas
		where o.codigo_oficina is null
...
select pr.* from pedido pd 
	left join detalle_pedido dp
	on pd.codigo_pedido = dp.codigo_pedido
	right join producto pr
...
select distinct o.*
	...
	and o.codigo_oficina not in (select distinct o.codigo_oficina
		from oficina o
		left join empleado em
		on o.codigo_oficina = em.codigo_oficina
		left join cliente cl
		...
		on dp.codigo_producto = pr.codigo_producto
			and pr.gama != 'Frutales'
			where pr.codigo_producto is not null); 
        
/*9. Devuelve un listado con los clientes que han realizado algún pedido pero no han
realizado ningún pago.*/
select cl.* from pedido pd
	join cliente cl
	on pd.codigo_cliente = cl.codigo_cliente
	left join pago pg
	on cl.codigo_cliente = pg.codigo_cliente
...
select e.*, em.nombre'Nombre Jefe'
	from cliente cl 
...
select count(*)'Cantidad clientes', pais from cliente group by pais;

#3. ¿Cuál fue el pago medio en 2009?
select avg(total)'pago medio' from pago where year(fecha_pago) = '2009';
 
 /*4. ¿Cuántos pedidos hay en cada estado? Ordena el resultado de forma descendente
por el número de pedidos.*/
select count(*) as 'Número de pedidos', estado from pedido 
	group by estado order by count(*) desc;
    
/*5. Calcula el precio de venta del producto más caro y más barato en una misma
consulta.*/
select max(precio_venta)'Precio producto mas caro',
	min(precio_venta)'Precio producto mas barato' 
    from producto;
...
select count(*)'Cantidad de clientes', ciudad from cliente where ciudad = 'Madrid';

#8. ¿Calcula cuántos clientes tiene cada una de las ciudades que empiezan por M?
select count(*)'Cantidad de clientes', ciudad from cliente where ciudad like 'M%'
	group by ciudad;
 
/*9. Devuelve el nombre de los representantes de ventas y el número de clientes al que
atiende cada uno.*/
select e.nombre'Rep. de ventas.\nNombre:', 
	count(cl.codigo_cliente)'Num. clientes que atiende' 
...
select max(pg.fecha_pago)'Fecha primer pago', 
	min(pg.fecha_pago)'Fecha último pago',
	cl.nombre_cliente'Nombre y apellido'
	from pago pg, cliente cl
		where pg.codigo_cliente = cl.codigo_cliente
		group by pg.codigo_cliente;

#12. Calcula el número de productos diferentes que hay en cada uno de los pedidos.
select codigo_pedido'Cod. pedido',
	count(codigo_producto)'Cantidad de productos'
	from detalle_pedido
...
uno de los pedidos.*/
select codigo_pedido'Cod. pedido',
  sum(cantidad * precio_unidad)'Total en cada pedido'
  from detalle_pedido
	group by codigo_pedido
	order by codigo_pedido;
...
select codigo_producto, sum(cantidad)'Cantidad vendida' 
	from detalle_pedido
	group by codigo_producto
	order by sum(cantidad) desc
	limit 20; 
 
/*15. La facturación que ha tenido la empresa en toda la historia, indicando la base
imponible, el IVA y el total facturado. La base imponible se calcula sumando el coste
del producto por el número de unidades vendidas de la tabla detalle_pedido. El IVA
es el 21 % de la base imponible, y el total la suma de los dos campos anteriores.*/
select sum(cantidad * precio_unidad)'Base imponible',
	truncate(sum(cantidad * precio_unidad) * 0.21, 2)'IVA',
	truncate(sum(cantidad * precio_unidad) * 1.21, 2)'Total facturado'
	from detalle_pedido;

/*16. La misma información que en la pregunta anterior, pero agrupada por código de
producto.*/
select codigo_producto'Cod. producto', 
	sum(cantidad * precio_unidad)'Base imponible',
	truncate(sum(cantidad * precio_unidad) * 0.21, 2)'IVA',
	truncate(sum(cantidad * precio_unidad) * 1.21, 2)'Total facturado'
	from detalle_pedido
	group by codigo_producto;
...
impuestos (21% IVA)*/
select pr.nombre'Producto nombre',
	sum(dp.cantidad)'Unidades vendidas', 
	sum(dp.cantidad * dp.precio_unidad)'Total facturado',
	truncate(sum(dp.cantidad * dp.precio_unidad) * 1.21, 2)'Total con IVA'
	from detalle_pedido dp, producto pr
		where dp.codigo_producto = pr.codigo_producto
		group by dp.codigo_producto
		having sum(dp.cantidad * dp.precio_unidad) * 1.21 > 3000
		order by sum(dp.cantidad * dp.precio_unidad) * 1.21 desc;
...
select nombre'Nombre producto' from producto
...
utilizar INNER JOIN).*/
select cl.* from cliente cl, pago pg
	where cl.codigo_cliente = pg.codigo_cliente
	group by pg.codigo_cliente
	having cl.limite_credito > sum(pg.total);
# Pendiente, intentar hacer mediante subconsulta

#5. Devuelve el/los producto/s que más unidades tiene en stock.
...
Alberto Soria.*/
select codigo_empleado from empleado where nombre = 'Alberto'
	and apellido1 = 'Soria';
	select nombre, apellido1, apellido2, email from empleado
	where codigo_jefe = (select codigo_empleado from empleado where nombre = 'Alberto'
	and apellido1 = 'Soria');
...
#1. Devuelve el nombre del cliente con mayor límite de crédito.
select nombre_cliente from cliente
	where limite_credito = any (select max(cl2.limite_credito) from cliente cl2); 

#2. Devuelve el nombre del producto que tenga el precio de venta más caro.
select nombre, codigo_producto from producto 
	where precio_venta = some (select max(precio_venta) from producto);          
...
/*1. Devuelve el nombre, apellido1 y cargo de los empleados que no representen a
ningún cliente.*/
select nombre'Nombre', apellido1'Apellido', puesto'Cargo' 
	from empleado 
...
/*3. Devuelve un listado que muestre solamente los clientes que sí han realizado ningún
pago.*/
select * from cliente 
...
select em.nombre'Nombre', em.apellido1'Primer apellido',
		em.apellido2'Segundo apellido', em.puesto'Puesto',
        o.telefono'Tel. oficina' 
...
/*1. Devuelve un listado que muestre solamente los clientes que no han realizado
ningún pago.*/
select cl.* from cliente cl where not exists (select pg.codigo_cliente from pago pg 
									where pg.codigo_cliente = cl.codigo_cliente);
                                    
#2. Devuelve un listado que muestre solamente los clientes que sí han realizado pago.
select cl.* from cliente cl where exists (select pg.codigo_cliente from pago pg 
									where pg.codigo_cliente = cl.codigo_cliente);
                                    
#3. Devuelve un listado de los productos que nunca han aparecido en un pedido.
select pr.* from producto pr where not exists (select dp.codigo_producto from detalle_pedido dp
									where pr.codigo_producto = dp.codigo_producto);

#4. Devuelve un listado de los productos que han aparecido en un pedido alguna vez.
select pr.* from producto pr where exists (select dp.codigo_producto from detalle_pedido dp
									where pr.codigo_producto = dp.codigo_producto);                                    
